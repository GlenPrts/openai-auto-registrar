...
WORK_DIR_ENV = "CPA_WORK_DIR"
WORK_DIR_POINTER_NAME = ".cpa_work_dir"
DEFAULT_WORK_DIR_NAME = "cpa_wokers"
CONFIG_CANDIDATES = ("cpa_config.json", "cpa_config.json.txt")
DEFAULT_WORK_FILES = (
    "invalid_codex_accounts.json",
    "invalid_quota_accounts.json",
    "active_quota_history.json",
    "active_quota_history.jsonl",
    "standby_accounts.json",
)
...
def normalize_work_dir(raw_path, base_dir):
...
def read_work_dir_pointer(base_dir):
    pointer = Path(base_dir) / WORK_DIR_POINTER_NAME
...
def clear_work_dir_pointer(base_dir):
    pointer = Path(base_dir) / WORK_DIR_POINTER_NAME
    if not pointer.exists():
        return
    try:
...
def write_work_dir_pointer(base_dir, work_dir):
    pointer = Path(base_dir) / WORK_DIR_POINTER_NAME
    try:
        pointer.write_text(f"{work_dir}\n", encoding="utf-8")
    except Exception:
        return


def default_work_dir(base_dir):
    return str((Path(base_dir) / DEFAULT_WORK_DIR_NAME).resolve())


def collect_extra_migration_paths(base_dir):
    legacy_cfg = pick_existing_in(base_dir, *CONFIG_CANDIDATES)
    conf = load_json_dict(legacy_cfg)
    keys = (
        "output",
        "quota_output",
        "active_quota_output",
...
def migrate_relative_path(src_base, dst_base, rel_path):
    src = Path(src_base) / rel_path
    if not src.exists():
        return
...
def resolve_work_dir(base_dir):
    env_path = str(os.environ.get(WORK_DIR_ENV) or "").strip()
    if env_path:
        return normalize_work_dir(env_path, base_dir)

    pointer_path = read_work_dir_pointer(base_dir)
    if pointer_path:
        pointer_dir = normalize_work_dir(pointer_path, base_dir)
        if os.path.abspath(pointer_dir) != os.path.abspath(base_dir):
            return pointer_dir
        clear_work_dir_pointer(base_dir)

    legacy_cfg = pick_existing_in(base_dir, *CONFIG_CANDIDATES)
    conf = load_json_dict(legacy_cfg)
    configured = str(conf.get("work_dir") or "").strip()
    if configured:
        return normalize_work_dir(configured, base_dir)

    return default_work_dir(base_dir)


def resolve_paths_and_migrate():
    base_dir = runtime_base_dir()
    default_dir = default_work_dir(base_dir)
    work_dir = resolve_work_dir(base_dir)
    Path(work_dir).mkdir(parents=True, exist_ok=True)
    extra_paths = collect_extra_migration_paths(base_dir)

    if os.path.abspath(work_dir) != os.path.abspath(base_dir):
        for name in CONFIG_CANDIDATES:
            migrate_relative_path(base_dir, work_dir, Path(name))
        for name in DEFAULT_WORK_FILES:
            migrate_relative_path(base_dir, work_dir, Path(name))
        for rel in extra_paths:
            migrate_relative_path(base_dir, work_dir, rel)

    if os.path.abspath(work_dir) != os.path.abspath(default_dir):
        write_work_dir_pointer(base_dir, work_dir)
    if os.path.abspath(work_dir) == os.path.abspath(default_dir):
        clear_work_dir_pointer(base_dir)
    config_path = pick_existing_in(work_dir, *CONFIG_CANDIDATES)
    return work_dir, config_path


WORK_DIR, CONFIG_PATH = resolve_paths_and_migrate()
...
DEFAULT_ACTIVE_QUOTA_OUTPUT = "active_quota_history.json"
...
def load_config(path):
    ...
    if not isinstance(data, dict):
        raise RuntimeError("cpa_config.json 顶层必须是对象")
    return data
...
async def probe_accounts(
            ...
            try:
                async with sem:
                    async with session.post(
                        ...
                        if resp.status >= 400:
                            raise RuntimeError(
                                f"management api-call http {resp.status}: {text[:200]}"
                            )
    ...
    connector = aiohttp.TCPConnector(
        limit=max(1, workers), limit_per_host=max(1, workers)
    )
    ...
    async with aiohttp.ClientSession(
        connector=connector, timeout=client_timeout, trust_env=True
    ) as session:
        tasks = [
            asyncio.create_task(probe_one(session, sem, item))
...
async def check_quota_accounts(
    ...
    async def quota_one(session, sem, item):
        ...
        if not quota_marked_by_status and isinstance(sm_data, dict):
            ...
            if any(
                k in top_type or k in top_code or k in top_msg for k in limit_keywords
            ):
                quota_marked_by_status = True
                ...
                async with sem:
                    async with session.post(
                        ...
                        if sc == 200:
                            ...
                            rate_limit = (
                                usage_data.get("rate_limit")
                                or usage_data.get("rateLimit")
                                or {}
                            )
                            ...
                            def parse_window(name, win):
                                ...
                                return {
                                    ...
                                    "reset_at": pick_first_val(
                                        win, "reset_at", "resetAt"
                                    ),
                                    ...
                                    "limit_reached": pick_first_val(
                                        win, "limit_reached", "limitReached"
                                    ),
                                }
                            ...
                            with_seconds = [
                                ...
                                for w in windows
                                if isinstance(
                                    w.get("limit_window_seconds"), (int, float)
                                )
                            ]
                            if weekly_window is None and with_seconds:
                                weekly_window = max(
                                    with_seconds,
                                    key=lambda x: x["limit_window_seconds"],
                                )
                            ...
                            if short_window is None and len(windows) > 1:
                                for w in windows:
                                    if weekly_window is None or w.get(
                                        "name"
                                    ) != weekly_window.get("name"):
                                        short_window = w
                                        break
                            ...
                            if (
                                ...
                                and isinstance(
                                    weekly_window.get("limit_window_seconds"),
                                    (int, float),
                                )
                                and weekly_window.get("limit_window_seconds")
                                <= 6 * 3600
                            ):
                            ...
                            weekly_reset_at = (
                                weekly_window.get("reset_at") if weekly_window else None
                            )
                            short_used_percent = (
                                short_window.get("used_percent")
                                if short_window
                                else None
                            )
                            short_reset_at = (
                                short_window.get("reset_at") if short_window else None
                            )
                            ...
                            if used_percent is not None:
                                ...
                                if result["quota_source"] == "weekly":
                                    result["invalid_quota"] = (
                                        used_percent >= weekly_quota_threshold
                                    )
                                elif result["quota_source"] == "5hour":
                                    result["invalid_quota"] = (
                                        used_percent >= primary_quota_threshold
                                    )

                            # 无 used_percent 时，使用 remaining/limit_reached/allowed 兜底识别额度耗尽
                            if result.get("used_percent") is None:
                                remaining_zero = any(
                                    w.get("remaining") == 0 for w in windows
                                )
                                weekly_limit_reached = bool(
                                    weekly_window
                                    and weekly_window.get("limit_reached") is True
                                )
                                short_limit_reached = bool(
                                    short_window
                                    and short_window.get("limit_reached") is True
                                )
                                rate_limit_reached = (
                                    pick_first_val(
    ...
    async with aiohttp.ClientSession(
        ...
        tasks = [
            asyncio.create_task(quota_one(session, sem, item))
            for item in refreshed_candidates
        ]
        for t in asyncio.as_completed(tasks):
            out.append(await t)
...
async def close_names(base_url, token, names, close_workers, timeout):
    return await set_disabled_names(
        base_url, token, names, True, close_workers, timeout
    )


async def enable_names(base_url, token, names, enable_workers, timeout):
    return await set_disabled_names(
        base_url, token, names, False, enable_workers, timeout
    )
...
async def delete_names(base_url, token, names, delete_workers, timeout):
        ...
        try:
            async with sem:
                async with session.delete(
                    url, headers=mgmt_headers(token), timeout=timeout
                ) as resp:
                    text = await resp.text()
                    data = safe_json_text(text)
                    ok = resp.status == 200 and data.get("status") == "ok"
    ...
    connector = aiohttp.TCPConnector(
        limit=max(1, delete_workers), limit_per_host=max(1, delete_workers)
    )
    client_timeout = aiohttp.ClientTimeout(total=max(1, timeout))
...
class EnhancedUI(tk.Tk):
    def __init__(self, conf, config_path, work_dir):
        super().__init__()
        ...
        self.conf["work_dir"] = str(self.work_dir)
        self.all_accounts = []
    ...
    def _init_config_vars(self):
        ...
        self.token_var = tk.StringVar(
            value=str(self.conf.get("token") or self.conf.get("cpa_password") or "")
        )
        self.workers_var = tk.StringVar(
            value=str(self.conf.get("workers") or DEFAULT_WORKERS)
        )
        self.quota_workers_var = tk.StringVar(
            value=str(self.conf.get("quota_workers") or DEFAULT_QUOTA_WORKERS)
        )
        self.delete_workers_var = tk.StringVar(
            value=str(self.conf.get("delete_workers") or DEFAULT_DELETE_WORKERS)
        )
        # Separate thresholds for weekly and 5-hour quotas
        self.weekly_quota_threshold_var = tk.StringVar(
            value=str(
                self.conf.get("weekly_quota_threshold") or DEFAULT_QUOTA_THRESHOLD
            )
        )
        self.primary_quota_threshold_var = tk.StringVar(
            value=str(
                self.conf.get("primary_quota_threshold") or DEFAULT_QUOTA_THRESHOLD
            )
        )
        ...
        self.auto_allow_closed_scan_var = tk.BooleanVar(
            value=bool(self.conf.get("auto_allow_scan_closed", False))
        )
    ...
    def _setup_styles(self):
        ...
        style.configure(
            "CardInner.TFrame", background=card_bg, relief="flat", borderwidth=0
        )
    ...
    def _build(self):
        ...
        ttk.Label(header, textvariable=self.status, style="Subtle.TLabel").grid(
        ...
        self.help_btn = ttk.Button(
            top_actions,
            text="使用说明",
            command=self._toggle_help_page,
            style="Neutral.TButton",
        )
        self.help_btn.pack(side="left", padx=(8, 0))
        ttk.Button(
            top_actions, text="关闭窗口", command=self.destroy, style="Neutral.TButton"
        ...
        cfg = ttk.LabelFrame(
            config_wrap, text="连接与检测参数", style="Card.TLabelframe", padding=(8, 5)
        )
        ...
        ttk.Label(cfg_top_row, text="管理端地址 (base_url)").grid(
            row=0, column=0, sticky="w"
        )
        ttk.Entry(cfg_top_row, textvariable=self.base_url_var).grid(
            row=0, column=1, sticky="ew", padx=(8, 18)
        )

        ttk.Label(cfg_top_row, text="访问令牌 (token / cpa_password)").grid(
            row=0, column=2, sticky="w"
        )
        ttk.Entry(cfg_top_row, textvariable=self.token_var, show="*").grid(
        ...
        ttk.Entry(params_row, textvariable=self.workers_var, width=8).pack(
            side="left", padx=(6, 14)
        )
        ttk.Label(params_row, text="额度检测并发").pack(side="left")
        ttk.Entry(params_row, textvariable=self.quota_workers_var, width=8).pack(
            side="left", padx=(6, 14)
        )
        ttk.Label(params_row, text="删除并发").pack(side="left")
        ttk.Entry(params_row, textvariable=self.delete_workers_var, width=8).pack(
            side="left", padx=(6, 22)
        )

        ttk.Label(params_row, text="周额度阈值 (%)").pack(side="left")
        ttk.Entry(
            params_row, textvariable=self.weekly_quota_threshold_var, width=8
        ).pack(side="left", padx=(6, 14))
        ttk.Label(params_row, text="5 小时额度阈值 (%)").pack(side="left")
        ttk.Entry(
            params_row, textvariable=self.primary_quota_threshold_var, width=8
        ).pack(side="left", padx=(6, 14))
        ttk.Label(params_row, text="活跃账号目标数").pack(side="left")
        ttk.Entry(params_row, textvariable=self.auto_keep_active_var, width=7).pack(
            side="left", padx=(6, 0)
        )

        auto = ttk.LabelFrame(
            config_wrap, text="自动巡检设置", style="Card.TLabelframe", padding=(8, 5)
        )
        ...
        ttk.Entry(auto_row1, textvariable=self.auto_interval_var, width=7).pack(
            side="left", padx=(6, 14)
        )
        ...
        ttk.Label(auto, textvariable=self.auto_status_var, style="Subtle.TLabel").pack(
            side="left", pady=(7, 0)
        )
        ...
        self.auto_allow_closed_scan_var.trace_add(
            "write", lambda *_: self._save_config()
        )

        # 运行参数改动后也持久化到 cpa_config.json
        ...
        self.weekly_quota_threshold_var.trace_add(
            "write", lambda *_: self._save_config()
        )
        self.primary_quota_threshold_var.trace_add(
            "write", lambda *_: self._save_config()
        )
        ...
        ttk.Entry(filter_frame, textvariable=self.filter_var, width=34).pack(
            side="left", padx=(6, 16)
        )
        ...
        ttk.Label(
            filter_frame, text="提示：双击表格行可切换勾选状态", style="Subtle.TLabel"
        ).pack(side="left")

        ops = ttk.LabelFrame(
            self.main_content, text="批量操作", style="Card.TLabelframe", padding=(8, 5)
        )
        ...
        ops_row.pack(fill="x")
        ttk.Button(
            ops_row, text="全选", command=self.select_all, style="Neutral.TButton"
        ).pack(side="left")
        ttk.Button(
            ops_row, text="取消全选", command=self.select_none, style="Neutral.TButton"
        ).pack(side="left", padx=(6, 10))
        ttk.Button(
            ops_row, text="检测401无效", command=self.check_401, style="Primary.TButton"
        ).pack(side="left", padx=(0, 6))
        ttk.Button(
            ops_row, text="检测额度", command=self.check_quota, style="Primary.TButton"
        ).pack(side="left", padx=6)
        ttk.Button(
            ops_row,
            text="检测（401+额度）",
            command=self.check_both,
            style="Primary.TButton",
        ).pack(side="left", padx=6)
        ttk.Button(
            ops_row,
        ...
        ttk.Button(
            ops_row,
            text="恢复已关闭",
            command=self.recover_closed_accounts,
            style="Neutral.TButton",
        ).pack(side="left", padx=6)
        ttk.Button(
            ops_row,
            text="加入备用池",
            command=self.add_selected_to_standby,
            style="Neutral.TButton",
        ).pack(side="left", padx=6)
        ttk.Button(
            ops_row,
            text="移出备用池",
            command=self.remove_selected_from_standby,
            style="Neutral.TButton",
        ).pack(side="left", padx=6)
        ttk.Button(
            ops_row,
        ...
        ttk.Label(ops, textvariable=self.action_progress, style="Subtle.TLabel").pack(
            fill="x", pady=(6, 0)
        )
        ...
        self.tree = ttk.Treeview(
            table_panel, columns=columns, show="headings", height=24
        )
        ...
        help_scroll = ttk.Scrollbar(
            self.help_page, orient="vertical", command=self.help_canvas.yview
        )
        ...
        self._help_window = self.help_canvas.create_window(
            (0, 0), window=self.help_inner, anchor="nw"
        )
        ...
        help_card = ttk.LabelFrame(
            self.help_inner, text="使用说明", style="Card.TLabelframe", padding=(14, 12)
        )
        help_card.pack(fill="both", expand=True)

        ttk.Label(help_card, text="一、快速上手", style="Header.TLabel").pack(
            anchor="w", pady=(0, 6)
        )
        ttk.Label(
            help_card,
            text="1) 填写“管理端地址(base_url)”和“访问令牌(token/cpa_password)”。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ...
        ttk.Label(
            help_card,
            text="3) 活跃账号目标数（全局参数）：在自动巡检、检测、移出备用、恢复已关闭等涉及开启账号的流程中都会生效；系统会把活跃账号控制在目标数以内。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ttk.Label(
            help_card,
            text="4) 不足时允许从已关闭账号补齐（全局参数）：开启后，备用池不够时会继续扫描已关闭账号。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ttk.Label(
            help_card,
            text="5) 先执行“检测401无效 / 检测额度 / 联合检测”，再进行关闭、恢复、删除。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ttk.Label(
            help_card,
            text="6) 401无效账号推荐删除 / 无额度账号推荐关闭 / 错误账号通常不需要额外操作",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ttk.Label(
            help_card,
            text="7) 可用关键词+状态筛选；双击表格行可勾选/取消勾选。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ...
        ttk.Label(help_card, text="二、状态与筛选说明", style="Header.TLabel").pack(
            anchor="w", pady=(0, 6)
        )
        ttk.Label(
            help_card,
            text="- 状态判定采用统一优先级（从高到低）：401无效 > 备用 > 已关闭 > 额度耗尽 > 活跃 > 错误 > 未知。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ...
        ttk.Label(
            help_card,
            text="- 未知：新导入的账号，尚未完成有效检测（建议优先检测）。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ttk.Label(
            help_card,
            text="- 已关闭：手动关闭的账号，可通过点击“恢复已关闭”按钮检测后恢复使用。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ...
        ttk.Label(
            help_card,
            text="- 若账号同时命中多个状态（如既在备用池又401失效），界面会优先显示“401无效”。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ...
        ttk.Label(help_card, text="三、备用池说明", style="Header.TLabel").pack(
            anchor="w", pady=(0, 6)
        )
        ttk.Label(
            help_card,
            text="- 加入备用池：将选中账号加入备用列表，并立即关闭账号。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ttk.Label(
            help_card,
            text="- 移出备用池：会先做 401+额度检测，仅“状态正常且非额度耗尽”的账号会开启并移入活跃账号池，并且移出备用池。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ttk.Label(
            help_card,
            text="- 自动补齐活跃账号时，系统会优先从备用池挑选可恢复账号。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x", pady=(0, 8))
        ...
        ttk.Label(
            help_card, text="四、自动巡检说明（重点）", style="Header.TLabel"
        ).pack(anchor="w", pady=(0, 6))
        ttk.Label(
            help_card,
            text="- 巡检间隔：按分钟周期执行。启动后会先立即执行一次。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ttk.Label(
            help_card,
            text="- 401处理：删除/仅标记；额度处理：关闭/删除/仅标记（按你的下拉选项执行）。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x", pady=(0, 8))
        ...
        ttk.Label(help_card, text="五、复杂操作与风险提示", style="Header.TLabel").pack(
            anchor="w", pady=(0, 6)
        )
        ttk.Label(
            help_card,
            text="- 推荐流程：联合检测（401+额度） → 核对状态 → 批量关闭/恢复/删除。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ttk.Label(
            help_card,
            text="- “并发值”：并发值越高检测速度越快，性能消耗越大（消耗的是cliproxy api所在机器的能），推荐自动巡检时设置小并发数。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
        ).pack(anchor="w", fill="x")
        ...
        ttk.Label(
            help_card,
            text="- 如检测结果异常，先检查 base_url、token 和网络连通性，再重试。",
            style="Subtle.TLabel",
            justify="left",
            wraplength=1060,
    ...
    def _save_config(self):
        try:
            ...
            weekly_quota_threshold_text = str(
                self.weekly_quota_threshold_var.get() or ""
            ).strip()
            try:
            ...
            primary_quota_threshold_text = str(
                self.primary_quota_threshold_var.get() or ""
            ).strip()
            try:
            ...
            self.conf["auto_allow_scan_closed"] = bool(
                self.auto_allow_closed_scan_var.get()
            )
            self.conf["standby_output"] = (
                self.conf.get("standby_output") or DEFAULT_STANDBY_OUTPUT
            )
            ...
            messagebox.showwarning("保存配置失败", f"配置未写入 cpa_config.json:\n{e}")
    ...
    def _ensure_accounts_loaded(self, action_name):
        if self.all_accounts:
            return True
        messagebox.showinfo(
            action_name, "当前账号列表为空，请先点击“刷新”并确保加载成功。"
        )
        return False
    ...
    def _runtime(self):
        ...
        output_path = self._resolve_output_path(
            self.conf.get("output") or DEFAULT_OUTPUT
        )
        quota_path = self._resolve_output_path(
            self.conf.get("quota_output") or DEFAULT_QUOTA_OUTPUT
        )
        active_path = self._resolve_output_path(
            self.conf.get("active_quota_output") or DEFAULT_ACTIVE_QUOTA_OUTPUT
        )
        ...
        return {
            ...
            "close_workers": int(
                self.conf.get("close_workers") or DEFAULT_CLOSE_WORKERS
            ),
            "enable_workers": int(
                self.conf.get("enable_workers") or DEFAULT_ENABLE_WORKERS
            ),
            "delete_workers": int(
                self.delete_workers_var.get() or DEFAULT_DELETE_WORKERS
            ),
            ...
            "target_type": (
                self.conf.get("target_type") or DEFAULT_TARGET_TYPE
            ).lower(),
            "provider": (self.conf.get("provider") or "").lower(),
            "weekly_quota_threshold": int(
                self.weekly_quota_threshold_var.get() or DEFAULT_QUOTA_THRESHOLD
            ),
            "primary_quota_threshold": int(
                self.primary_quota_threshold_var.get() or DEFAULT_QUOTA_THRESHOLD
            ),
            "output": str(output_path),
            "quota_output": str(quota_path),
            "active_quota_output": str(active_path),
    ...
    def _rebalance_active_target_by_runtime(self, rt):
        ...
        try:
            ...
            primary_candidates = self._collect_primary_auto_candidates_from_files(
                files, rt
            )
            summary["active_before"] = len(primary_candidates)

            overflow_result = self._move_active_overflow_to_standby(
                rt, primary_candidates, target_active
            )
            summary["moved_to_standby_count"] = len(
            ...
            files = fetch_auth_files(rt["base_url"], rt["token"], rt["timeout"])
            current_primary = self._collect_primary_auto_candidates_from_files(
                files, rt
            )
            ...
            if need_count > 0:
                standby_candidates = self._collect_standby_candidates_from_files(
                    files, rt
                )
                standby_scan = self._scan_for_recovery(
                    rt, standby_candidates, need_count
                )
                ...
                standby_scanned_names = set(standby_probe_by_name.keys()) | set(
                    standby_quota_by_name.keys()
                )
                if standby_scanned_names:
                    self._apply_scan_maps_to_accounts(
                        standby_scanned_names,
                        standby_probe_by_name,
                        standby_quota_by_name,
                    )

                standby_invalid_401 = {
                    n for n in (standby_scan.get("invalid_401") or []) if n
                }
                standby_invalid_quota = {
                    n for n in (standby_scan.get("invalid_quota") or []) if n
                }
                ...
                if remaining_need > 0 and rt.get("auto_allow_scan_closed"):
                    closed_candidates = self._collect_closed_candidates_from_files(
                        files, rt, exclude_names=picked_names
                    )
                    closed_scan = self._scan_for_recovery(
                        rt, closed_candidates, remaining_need
                    )
                    ...
                    closed_scanned_names = set(closed_probe_by_name.keys()) | set(
                        closed_quota_by_name.keys()
                    )
                    if closed_scanned_names:
                        self._apply_scan_maps_to_accounts(
                            closed_scanned_names,
                            closed_probe_by_name,
                            closed_quota_by_name,
                        )
        ...
        def worker():
            try:
                ...
                for f in files:
                    ...
                    stream_error_active = _is_stream_error_active(
                        raw_status, status_msg
                    )
                    ...
                    if normalized_status == "error" and status_msg:
                        ...
                        if err.get("type") == "usage_limit_reached" and err.get(
                            "resets_at"
                        ):
                            try:
                    ...
                    if pending_scan:
                        ...
                        account_status = (
                            prev.get("status")
                            if prev.get("status") in ("active", "error")
                    ...
                    accounts.append(
                        {
                            ...
                            "individual_used_percent": prev.get(
    ...
    def _display_usage(self, account):
        ...
        if used_percent is not None:
            ...
            if account.get("reset_at"):
                try:
                    reset_time = datetime.fromtimestamp(account["reset_at"]).strftime(
                        "%Y-%m-%d %H:%M"
                    )
                except Exception:
                    reset_time = ""
        ...
        for idx, account in enumerate(filtered):
            ...
            self.tree.insert(
                ...
                values=(
                    ...
                    (account.get("check_error") or account.get("error_info") or "")[
                        :160
                    ],
                ),
            )
    ...
    def _selected_names(self):
        return [
            ...
            if a.get("_selected") and a.get("name")
        ]
    ...
    def _add_standby_done(self, close_results):
        ok = [
            r.get("name") for r in close_results if r.get("updated") and r.get("name")
        ]
        ...
        if bad:
            msg = "以下账号加入备用失败：\n" + "\n".join(
                [f"- {r.get('name')}: {r.get('error')}" for r in bad[:15]]
            )
            if len(bad) > 15:
                msg += f"\n... 还有 {len(bad) - 15} 条"
    ...
    def remove_selected_from_standby(self):
        ...
        self.action_progress.set(
            f"正在检测备用账号并尝试移出... 候选={len(candidates)}"
        )
        ...
        def worker():
            try:
                ...
                probe_by_name = {
                    r.get("name"): r for r in probe_results if r.get("name")
                }
                quota_by_name = {
                    r.get("name"): r for r in quota_results if r.get("name")
                }
                ...
                for raw in candidates:
                    ...
                    if not name:
                        continue
                    if self._is_recoverable_by_scan(
                        probe_by_name.get(name), quota_by_name.get(name)
                    ):
                        recoverable_names.append(name)

                limit_meta = self._pick_names_with_active_target_limit(
                    recoverable_names
                )
    ...
    def _remove_standby_scan_done(
        ...
        for account in self.all_accounts:
            ...
            if invalid_401:
                ...
                if account.get("raw"):
                    account["raw"]["disabled"] = False
                account["check_error"] = (p.get("error") if p else None) or (
                    q.get("error") if q else None
                )
                continue
            ...
            if invalid_quota:
                ...
                if account.get("raw"):
                    account["raw"]["disabled"] = True
                account["check_error"] = (q.get("error") if q else None) or (
                    p.get("error") if p else None
                )
                continue
        ...
        self.action_progress.set(
            f"移出备用完成：活跃={active_count} 401={moved_401_count} 已关闭={moved_closed_count}"
        )
    ...
    def _apply_scan_maps_to_accounts(self, names, probe_by_name, quota_by_name):
        names_set = {
            str(n or "").strip() for n in (names or []) if str(n or "").strip()
        }
        if not names_set:
            return
            ...
            for item in chunk:
                ...
                if self._is_recoverable_by_scan(
                    ...
                    if (
                        ...
                        and len(recoverable_names) >= target_count
                    ):
                        break
    ...
    def _scan_active_candidates_and_apply(self, rt, active_candidates, need_count=None):
        ...
        if target_count is None:
            chunk_size = max(
                1, min(len(active_candidates), max(1, rt.get("workers", 1)))
            )
        else:
            batch_size = self._target_scan_batch_size(rt)
        ...
        for i in range(0, len(active_candidates), chunk_size):
            ...
            for item in chunk:
                ...
                if self._is_recoverable_by_scan(
                    ...
                    if (
                        target_count is not None
                        and len(active_ok_names) >= target_count
                    ):
                        break
    ...
    def _apply_auto_401_action_only(self, rt, invalid_401_names):
        ...
        delete_results = asyncio.run(
            delete_names(
                rt["base_url"], rt["token"], names, rt["delete_workers"], rt["timeout"]
            )
        )
    ...
    def _auto_apply_actions(
        self, rt, invalid_401_names, invalid_quota_names, limit_only_quota_names
    ):
        ...
        if action_quota == "删除":
            ...
            to_delete.update(
                [n for n in invalid_quota_names if n not in limit_only_set]
            )
    ...
    def _run_scheduled_check_once(self):
        ...
        def worker():
            ...
            try:
                ...
                active_scan = self._scan_active_candidates_and_apply(
                    rt, primary_candidates, need_count=target_active
                )

                detected_active_names = [
                    n for n in (active_scan.get("active_ok") or []) if n
                ]
                ...
                unscanned_active_names = [
                    n for n in active_name_order if n not in set(detected_active_set)
                ]
                ...
                unknown_candidates = self._collect_unknown_candidates_from_files(
                    files, rt
                )
                unknown_scan = self._scan_for_recovery(
                    rt, unknown_candidates, need_count=None
                )

                unknown_invalid_401 = sorted(set(unknown_scan.get("invalid_401") or []))
                unknown_invalid_quota = sorted(
                    set(unknown_scan.get("invalid_quota") or [])
                )
                unknown_limit_only_quota = sorted(
                    set(unknown_scan.get("limit_only_quota") or [])
                )
                ...
                unknown_recoverable_names = [
                    ...
                    if n
                    and n not in deleted_unknown_set
                    and n not in closed_unknown_set
                ]
                ...
                replenish_files = fetch_auth_files(
                    rt["base_url"], rt["token"], rt["timeout"]
                )
                current_primary_candidates = (
                    self._collect_primary_auto_candidates_from_files(
                        replenish_files, rt
                    )
                )
                ...
                if remaining_need > 0 and rt.get("auto_allow_scan_closed"):
                    ...
                    closed_scan = self._scan_for_recovery(
                        rt, closed_candidates, need_count=remaining_need
                    )
                    picked_names.extend(closed_scan.get("recoverable") or [])
                ...
                if picked_names:
                    ...
                    for r in enable_results:
                        ...
                        if r.get("updated"):
                            ...
                            if (
                                name
                                and name in standby_recoverable_set
                                and name in self.standby_names
                            ):
                                self.standby_names.remove(name)
                ...
                processed_401 = set(active_scan.get("invalid_401") or []) | set(
                    unknown_scan.get("invalid_401") or []
                )
                extra_invalid_401 = sorted(
                    {n for n in invalid_401_all if n not in processed_401}
                )
                extra_401_action_result = self._apply_auto_401_action_only(
                    rt, extra_invalid_401
                )

                processed_quota = set(active_scan.get("invalid_quota") or []) | set(
                    unknown_scan.get("invalid_quota") or []
                )
                extra_invalid_quota = sorted(
                    {n for n in invalid_quota_all if n not in processed_quota}
                )
                ...
                close_errors_all = (
                    list(active_scan.get("actions", {}).get("close_errors") or [])
                    + list(unknown_action_result.get("close_errors") or [])
                    + list(extra_quota_action_result.get("close_errors") or [])
                    + list(unknown_standby_errors)
                )
                delete_errors_all = (
                    list(active_scan.get("actions", {}).get("delete_errors") or [])
                    + list(unknown_action_result.get("delete_errors") or [])
                    + list(extra_401_action_result.get("errors") or [])
                    + list(extra_quota_action_result.get("delete_errors") or [])
                )
                ...
                # 兜底裁剪，确保最终活跃数不超过目标
                final_files_before_trim = fetch_auth_files(
                    rt["base_url"], rt["token"], rt["timeout"]
                )
                final_primary_before_trim = (
                    self._collect_primary_auto_candidates_from_files(
                        final_files_before_trim, rt
                    )
                )
                trim_result = {
                    "overflow_total": 0,
                ...
                if (
                    target_active >= 0
                    and len(final_primary_before_trim) > target_active
                ):
                    trim_result = self._move_active_overflow_to_standby(
                        rt, final_primary_before_trim, target_active
                    )
                    moved_to_standby_all = sorted(
                        {
                            n
                            for n in (
                                moved_to_standby_all
                                + (trim_result.get("moved_to_standby") or [])
                            )
                            if n
                        }
                    )
                ...
                final_files = fetch_auth_files(
                    rt["base_url"], rt["token"], rt["timeout"]
                )
                active_after_scan = len(
                    self._collect_primary_auto_candidates_from_files(final_files, rt)
                )

                standby_file_dirty = bool(unknown_standby_moved) or bool(
                    standby_removed_by_enable
                )
                deleted_set = set(deleted_all)
                if deleted_set:
                    removed_deleted = [
                        name for name in list(self.standby_names) if name in deleted_set
                    ]
                    if removed_deleted:
                ...
                if closed_set:
                    removed_closed = [
                        name for name in list(self.standby_names) if name in closed_set
                    ]
                    if removed_closed:
                ...
                summary = {
                    ...
                    "overflow_total": len(unscanned_active_names)
                    + int(trim_result.get("overflow_total") or 0),
    ...
    def _schedule_next_auto_check(self):
        ...
        self._auto_job = self.after(
            interval_minutes * 60 * 1000, self._run_scheduled_check_once
        )
    ...
    def check_401(self):
        ...
        if not candidates:
            if only_unknown:
                messagebox.showinfo(
                    "检测 401 失效",
                    "当前筛选为“未知”，没有符合条件（未知、未关闭、匹配 target_type/provider 且带 auth_index）的账号可检测。",
                )
            else:
                messagebox.showinfo(
                    "检测 401 失效",
                    "没有符合条件（活跃/未知/错误、未关闭、匹配 target_type/provider 且带 auth_index）的账号可检测。",
                )
            return
    ...
    def _check_401_done(self, results, output_path):
        ...
        self._apply_filter()
        self.action_progress.set(
            f"401 检测完成：无效={invalid_count} 异常={error_count}"
        )
        msg = (
    ...
    def _check_quota_done(self, results, output_path):
        ...
        self._apply_filter()
        self.action_progress.set(
            f"额度检测完成：额度耗尽={quota_count} 异常={error_count}"
        )
    ...
    def _check_both_done(
        self, probe_results, quota_results, output_path, quota_output_path
    ):
        probe_by_name = {r.get("name"): r for r in probe_results if r.get("name")}
        quota_by_name = {r.get("name"): r for r in quota_results if r.get("name")}
        ...
        self._apply_filter()
        self.action_progress.set(
            f"联合检测（401+额度）完成：401={count_401} 额度耗尽={count_quota}"
        )
        msg = (
            f"401无效: {count_401}\n"
            f"额度耗尽: {count_quota}\n"
    ...
    def recover_closed_accounts(self):
        ...
        self.action_progress.set(
            f"正在检测已关闭账号额度与状态... 候选={len(candidates)}"
        )
            ...
            try:
                ...
                for q in quota_results:
                    ...
                    if (
                        ...
                        or p.get("invalid_401")
                        or p.get("error")
                    ):
                        continue
                    if (
                        q.get("status_code") != 200
                        or q.get("invalid_quota")
                        or q.get("error")
                    ):
                        continue
    ...
    def _recover_closed_done(
        self, probe_results, quota_results, enable_results, limit_meta=None
    ):
        probe_by_name = {r.get("name"): r for r in probe_results if r.get("name")}
        quota_by_name = {r.get("name"): r for r in quota_results if r.get("name")}
        enable_by_name = {r.get("name"): r for r in enable_results if r.get("name")}
        ...
        for account in self.all_accounts:
            ...
            if q:
                ...
                if (
                    p_ok
                    and q.get("status_code") == 200
                    and not q.get("invalid_quota")
                    and not q.get("error")
                ):
                    recoverable += 1
            ...
            if e:
                if e.get("updated"):
                    ...
                    enabled_fail += 1
                    account["check_error"] = e.get("error") or account.get(
                        "check_error"
                    )

        self._apply_filter()
        self.action_progress.set(
            f"恢复流程完成：开启成功={enabled_ok} 失败={enabled_fail}"
        )
...
def main():
    ...
    app = EnhancedUI(conf, CONFIG_PATH, WORK_DIR)
    app.mainloop()
